  ...
  return selectedUser ? (
    <div className="h-full overflow-y-scroll relative backdrop-blur-lg">
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
            ...
            <div className="text-center text-xs">
              <img src={msg.senderId === authUser._id ? authUser._id?.profilePic || assets.avatar_icon : selectedUser?.profilePic ||  assets.profile_martin} alt="" className='w-7 rounded-full' />
      ...
      <div className="absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3">
        ...
        <img onClick={handleSendMessage} src={assets.send_button} alt="" className='w-12 cursor-poiter' />
      </div>
    </div>
  ) : (
